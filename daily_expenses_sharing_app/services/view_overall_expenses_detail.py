...
def view_overall_expenses_detail(current_user, db: Session):
    try:
        # Fetch the expenses data based on the current user's ID
        user_id = current_user.user_id
        expenses = db.query(Expense).options(joinedload(Expense.splits)).filter(Expense.user_id == user_id).all()
