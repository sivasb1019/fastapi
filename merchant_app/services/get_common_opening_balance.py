...
def get_common_opening_balance(input_date, session1, session2):
    try:
        session_data = get_id(session2)
        ob_instance = session1.execute(text("call common_opening_balance(:input_date, :branch_id, :merchant_id)"),
